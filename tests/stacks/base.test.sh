#!/usr/bin/env bash
# base.test.sh - Tests for base stack (traefik, portainer, watchtower)
# Copyright (c) 2026 homelab-stack contributors
# SPDX-License-Identifier: MIT

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "${SCRIPT_DIR}/../lib/assert.sh"
source "${SCRIPT_DIR}/../lib/docker.sh"
source "${SCRIPT_DIR}/../lib/report.sh"

STACK_NAME="base"
SERVICES=(traefik portainer watchtower)

setup() {
    assert_reset
    report_init "$STACK_NAME"
}

teardown() {
    report_write_json
    report_print_summary
}

# --- Tests ---

test_compose_file_exists() {
    assert_set_test "compose_file_exists"
    local compose_file
    compose_file=$(get_compose_file "$STACK_NAME")
    assert_file_exists "$compose_file" "base compose file should exist"
}

test_all_services_defined() {
    assert_set_test "all_services_defined"
    local compose_file
    compose_file=$(get_compose_file "$STACK_NAME")
    for svc in "${SERVICES[@]}"; do
        assert_service_exists "$compose_file" "$svc"
    done
}

test_traefik_running() {
    assert_set_test "traefik_running"
    assert_container_running "traefik" "traefik container should be running"
}

test_traefik_healthy() {
    assert_set_test "traefik_healthy"
    assert_container_healthy "traefik"
}

test_portainer_running() {
    assert_set_test "portainer_running"
    assert_container_running "portainer" "portainer container should be running"
}

test_portainer_http() {
    assert_set_test "portainer_http"
    local ip
    ip=$(get_container_ip "portainer")
    if [ -n "$ip" ]; then
        assert_http_200 "http://${ip}:9000" "portainer web UI should respond"
    else
        _assert_skip "portainer HTTP check" "could not determine container IP"
    fi
}

test_watchtower_running() {
    assert_set_test "watchtower_running"
    assert_container_running "watchtower" "watchtower container should be running"
}

# --- Run ---
setup
for func in $(declare -F | grep -o 'test_' | sort); do
    echo -e "\n${_C_CYAN}▶ ${func}${_C_RESET}"
    $func
done
teardown
