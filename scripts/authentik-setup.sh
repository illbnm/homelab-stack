...
create_provider() {
    ...
    local response=$(curl -s -X POST "${AUTHENTIK_API_URL}/providers/oauth2/" \
        ...
        -H "Content-Type: application/json" \
        -d '{"name": "'"$name"'", "client_type": "confidential", "authorization_flow": "default-resource-flow", "redirect_uris": ["'"$redirect_uri"'"]}')
...
echo "Setup complete. Please update your service configuration files with the provided Client IDs and Client Secrets."
