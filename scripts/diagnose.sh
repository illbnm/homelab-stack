...
collect_system_info() {
  ...
  local info
  info=$(cat <<EOF
操作系统: $(lsb_release -d 2>/dev/null | cut -f2 || cat /etc/os-release 2>/dev/null | grep PRETTY_NAME | cut -d\" -f2 || uname -s)
内核版本: $(uname -r)
架构: $(uname -m)
运行时间: $(uptime -p 2>/dev/null || uptime)

CPU:
$(lscpu | grep -E 'Model name|CPU\(s\)|CPU MHz|CPU max MHz' 2>/dev/null || echo "  无法获取 CPU 信息")
...
网络接口:
$(ip -brief addr show 2>/dev/null || ifconfig 2>/dev/null | grep -E '^[a-z]' | head -10)
EOF
)
...
EOF
  ...
  # Add logs for unhealthy/exited containers
  local containers
...
防火墙状态:
$(ufw status 2>/dev/null || firewall-cmd --state 2>/dev/null || echo "  无法检测防火墙状态")

开放端口:
$(ss -tlnp 2>/dev/null | grep -E ':(80|443|3000|8080|9000)\s' || netstat -tlnp 2>/dev/null | grep -E ':(80|443|3000|8080|9000)\s' || echo "  无法列出端口")
EOF
)
...
validate_configs() {
  ...
  if [[ -f "$PROJECT_ROOT/.env" ]]; then
    ...
    local required_vars=("DOMAIN" "ACME_EMAIL" "TRAEFIK_DASHBOARD_USER" "TRAEFIK_DASHBOARD_PASSWORD_HASH" "TZ")
  ...
  while IFS= read -r compose_file; do
    ...
    if docker compose -f "$compose_file" config --quiet 2>/dev/null; then
      validation+="  ✓ $(basename "$(dirname "$compose_file")")/$(basename "$compose_file")\n"
    else
      validation+="  ✗ $(basename "$(dirname "$compose_file")")/$(basename "$compose_file") - 配置错误\n"
    fi
  done < <(find "$PROJECT_ROOT/stacks" -name "docker-compose*.yml" 2>/dev/null | head -10)
