...
# Backup PostgreSQL
PGPASSWORD=$POSTGRES_ROOT_PASSWORD pg_dumpall -h postgres -U postgres > "$BACKUP_DIR/postgres_backup.sql"

# Backup Redis
redis-cli -h redis BGSAVE
cp /data/dump.rdb "$BACKUP_DIR/redis_backup.rdb"

# Create tar.gz archive
tar -czf "$BACKUP_FILE" -C "$BACKUP_DIR" postgres_backup.sql redis_backup.rdb

# Remove old backups (keep last 7 days)
find "$BACKUP_DIR" -type f -name "databases_backup_*.tar.gz" -mtime +7 -exec rm {} \;
