...
BACKUP_DIR="/backups"
TIMESTAMP=$(date +"%F_%T")
BACKUP_FILE="$BACKUP_DIR/homelab_databases_backup_$TIMESTAMP.tar.gz"

mkdir -p $BACKUP_DIR

PGPASSWORD=$POSTGRES_ROOT_PASSWORD pg_dumpall -h postgres -U postgres -f $BACKUP_DIR/postgres_backup.sql
